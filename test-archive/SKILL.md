...
**任何测试相关产出落到磁盘前，必须先存在 `wkq/NN_<简述>/` 批次目录。**

无论用户让你做哪一步——只写方案、只跑某个用例、只整理报告——都必须先执行 `python -m test_archive list`，决定 `new`（新建批次）或 `resume`（复用已有批次），**之后所有 Write/Edit 的文件路径必须以 `wkq/NN_xxx/{plan,data,report}/` 开头**。

❌ 错误：用户说"写个测试方案"，直接 `Write ./test_plan.md` 或写到工程其他位置
✅ 正确：先 `new "登录接口回归测试"` 创建批次，再 `Write wkq/01_登录接口回归测试/plan/test_plan.md`

**开工第一件事永远是建批次，不是写文件。** 没有 batch 目录，Write/Edit 一律拒绝执行。
...
| 用户说法 | 处理 |
|---|---|
| "测试一下"、"出方案"、"部署后跑"、**"只写方案"、"出个方案文档"、"先写测试方案"** | **新建**批次 |
| "继续跑之前的方案"、"补测"、"回归" | **复用**已有批次 |
| "整理报告"但已有方案与数据 | **复用**已有批次 |

判断不准时**先问用户**，不要臆测。

⚠ **特别提醒**：即使用户只要"写个方案"还没让你执行，也必须先建批次。绝不允许跳过批次目录直接 `Write ./test_plan.md`——那会污染工程仓库，正是本 skill 要杜绝的。

**正确顺序**：
1. `python -m test_archive list`（看现有批次）
2. `python -m test_archive new "<简述>"` 或 `resume <query>`
3. 拿到 `wkq/NN_xxx/plan/` 路径后，再 `Write` / `Edit` 方案文档

**只要还没拿到批次路径，禁止 Write/Edit 任何测试文件。**
